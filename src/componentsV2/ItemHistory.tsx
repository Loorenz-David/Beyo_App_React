...
const propHelper = (key,val,identifier) =>{
    return(
...
const objRawHelper = (objProp,key) =>{
    return ( 
...
const ItemHistoryPage = ({itemId,creationTime,creationUser,holdScrollElement})=>{
    const {doFetch,data,loading} = useFetch()

    useEffect(()=>{
        const fetchDict ={
            model_name:'Item',
            requested_data:[{'item_history':['recorded_time','to_value','from_value','column_name','user_name','type']}],
            query_filters:{'id':itemId},
            per_page:20
        }
        const setRules= {
            loadData:true
    ...
    const {properDate,properTime} = generateProperTime(creationTime)
   

    if(data.length > 0 && data[0].item_history.length > 0){
        data[0].item_history.sort((a,b)=> new Date(b.recorded_time).getTime() - new Date(a.recorded_time).getTime())
       
    }

    return(
        <div ref={holdScrollElement} className="flex-column " style={{height:'100dvh',padding:'30px 20px',overflowY:'auto'}}>
            {loading ? 
            ...
            : 
                <div className=" width100 gap-1" style={{position:'relative',display:'flex', flexDirection:'column'}} >
                    ...
                    {data.length > 0 && data[0].item_history.length > 0 && data[0].item_history.map((obj,i)=>{
                        ...
                        return(
                            <div key={`historyContainer_${i}`} className="flex-row items-center " style={{zIndex:3}}>
                                ...
                                <div className="flex-column  padding-20   width100 ">
                                    ...
                                    >
                                        <span className="text-15">{obj.type} {obj.column_name}. </span>
                        ...
                        <div className=" width100 gap-2   padding-left-10 expandable-row " style={{borderRadius:'5px'}}>   
                            <div className="flex-column gap-2 ">
                                ...
                                <div className="flex-row gap-4 padding-top-10">
                                    <div className="flex-column gap-05">
                                        <span className="text-12 color-lower-titles">By:</span>
                                        <span className="text-12">{creationUser ?? 'not specify'}</span>
                                    </div>
                                    <div className="flex-column gap-05">
                                        <span className="text-12 color-lower-titles">At:</span>
                                        <span className="text-12">{properTime}</span>
...
const ItemHistoryBtn = ({itemId,creationTime,creationUser=null,zIndex=2})=>{
    const [toggleItemHistory,setToggleItemHistory] = useState(false)

    return(
        <div className="flex-row items-center content-center">
            {toggleItemHistory && 

                <SecondaryPage BodyComponent={ItemHistoryPage}
                    bodyProps={{itemId,creationTime,creationUser}}
