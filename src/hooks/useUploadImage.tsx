...
async function generatePresignedUrl(listOfImages){
    const resUrls = await fetch('/api/generate-presigned-url',{
        method:'POST',
...
export async function deleteImageS3(listOfUrls){

    try{
        const fetchDict = {'urls':listOfUrls}

        const response =  await fetch('/api/delete-image-url',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
...
export async function UploadImage(listOfImages,filePath,attempt=1){
